...
func TestOpenAIRegistrationGateIsBlockedAndBounded(t *testing.T) {
	...
	wantURLs := map[string]bool{
		subscription.EvidenceAuthURL:                   true,
		subscription.EvidenceCodexPlanURL:              true,
		subscription.EvidenceEnterpriseAccessTokensURL: true,
		subscription.EvidenceSignInWithChatGPTURL:      true,
		subscription.EvidenceAPIQuickstartURL:          true,
	}
	allowedHosts := map[string]bool{
		"learn.chatgpt.com":   true,
		"help.openai.com":     true,
...
func TestOpenAIRegistrationCannotConstructOrLogin(t *testing.T) {
	t.Parallel()

	var zero subscription.RegistrationGate
	for name, gate := range map[string]subscription.RegistrationGate{
		"published gate": subscription.OpenAIRegistration(),
		"zero gate":      zero,
	} {
		name, gate := name, gate
...
func TestOpenAIRegistrationDoesNotExposeIdentityOrEndpoints(t *testing.T) {
	t.Parallel()
	...
	formatted := fmt.Sprintf("%#v", subscription.OpenAIRegistration())
	for _, forbidden := range []string{"evidenceURLs", "client_id", "client secret", "issuer", "audience", "redirect_uri", "endpoint", "origin", "grant", "scope", "api.openai.com"} {
