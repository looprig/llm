...
const (
	counterTokenizerRevision     contextcount.TokenizerRevision = "openai-responses-input-tokens-v1"
	openAISecurityPolicyRevision                                = "openai-api-bearer-tls-v1"
	maxCountResponseBodyBytes                                   = 64 << 10
	maxAPIErrorBodyBytes                                        = 1 << 20
	defaultCounterTimeout                                       = 60 * time.Second
)

// Counter is a separately constructed exact OpenAI Responses input-token
// counter. It is intentionally not embedded in the inference Client, so a
// normal client cannot acquire ContextCounter accidentally.
type Counter struct {
	endpoint    string
...
func (c *Counter) CountContext(ctx context.Context, req inference.Request) (contextcount.ContextCount, error) {
	...
	httpReq, err := http.NewRequestWithContext(counterCtx, http.MethodPost, strings.TrimRight(c.endpoint, "/")+"/responses/input_tokens", bytes.NewReader(body))
	if err != nil {
...
func (c *Counter) preflight(req inference.Request) ([]byte, error) {
	if llm.Provider(req.Model.Provider) != llm.ProviderOpenAI {
		return nil, &failure.ModelMismatchError{
			BoundProvider:   model.ProviderName(llm.ProviderOpenAI),
	...
	if req.Model.APIFormat != model.APIFormatOpenAIResponses {
		return nil, &model.ValidationError{Field: "APIFormat", Reason: "OpenAI counter requires openai-responses"}
	}
	encoded, err := (responses.Codec{}).EncodeRequest(req, 0)
	if err != nil {
	...
	// The count endpoint accepts the prompt side of a Responses request. Drop
	// generation controls so the counter request stays within its documented
	// input-token schema while preserving instructions, input, tools, and the
	// model that determine token occupancy.
	for field := range body {
		switch field {
		case "model", "instructions", "input", "tools", "tool_choice":
		default:
			delete(body, field)
		}
	}
...
func (c *Counter) CounterCapability() contextcount.CounterCapability {
	...
	return contextcount.CounterCapability{
		Provider:         contextcount.ProviderID(llm.ProviderOpenAI),
...
func counterSecurityIdentity(endpoint string) contextcount.SecurityIdentity {
	material := "provider=openai\nendpoint=" + endpoint + "\nauth=bearer\ntransport=tls\npolicy=" + openAISecurityPolicyRevision
