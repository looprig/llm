...
func (e *CounterStateError) Error() string {
	return "openai: invalid input-token counter state: " + string(e.Reason)
}

type CounterRequestReason string

const (
	CounterRequestEncodeFailed CounterRequestReason = "Responses request encoding failed"
...
func (e *CounterRequestError) Error() string {
	if e.Err != nil {
		return "openai: build input-token request: " + string(e.Reason) + ": " + e.Err.Error()
	}
	return "openai: build input-token request: " + string(e.Reason)
}
...
func (e *CounterEndpointError) Error() string {
	return "openai: invalid input-token counter endpoint: " + string(e.Reason)
}
...
func (e *CounterResponseFieldError) Error() string {
	return fmt.Sprintf("openai: input-token response field %s: %s", e.Field, e.Reason)
}
...
func (e *CounterResponseError) Error() string {
	if e.Err != nil {
		return "openai: input-token response: " + string(e.Reason) + ": " + e.Err.Error()
	}
	return "openai: input-token response: " + string(e.Reason)
}
