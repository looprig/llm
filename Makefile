...
# Format every Go file directly in each first-party package directory.
fmt:
	gofmt -w $(GO_FILES)
...
# The conformance gate and its generator moved to
# github.com/looprig/inference/codec/conformance, one tier down, so the codec
# encode paths the gate exists to check can reach it too. Regenerate the schema
# tree with `make conformance-schemas` in the inference module.
